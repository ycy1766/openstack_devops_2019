...
stage('Deploy_instance', {
  node('', {
    sh 'source  /etc/kolla/admin-openrc.sh  && export IMG_ID=`openstack image show packer-2 -c id -f value ` && cd  ./terraform && /usr/local/bin/terraform apply -var packer_image_name=$IMG_ID  '
